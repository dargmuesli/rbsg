...
public class SystemSocket extends AbstractWebSocket {
            ...
            switch(action) {
                ...
                case "gameCreated":
                    for(SystemSocketMessageHandler.SystemSocketGameCreateHandler handler : handlersGameCreate)
                        handler.handle(data.get("name").getAsString(), data.get("id").getAsString(), data.get("neededPlayer").getAsInt());
                    break;
                case "gameDeleted":
                    ...
                    break;
                default:
                    throw new IllegalArgumentException("Illegal action " + action + " recieved on System Socket!");
            }
        });
    }
    ...
    public void registerUserJoinHandler(SystemSocketMessageHandler.SystemSocketUserJoinHandler handler) {
        handlersUserJoin.add(handler);
    }
    
    public void registerUserLeftHandler(SystemSocketMessageHandler.SystemSocketUserLeftHandler handler) {
        handlersUserLeft.add(handler);
    }
    
    public void registerGameCreateHandler(SystemSocketMessageHandler.SystemSocketGameCreateHandler handler) {
        handlersGameCreate.add(handler);
    }
    
    public void registerGameDeleteHandler(SystemSocketMessageHandler.SystemSocketGameDeleteHandler handler) {
